...
export async function getEmote(dispatch, OAuth, streamerId) {
  const twitchApi = "https://api.twitch.tv/helix/chat/emotes?broadcaster_id=" + streamerId;
  axios
    .get(twitchApi, {
      headers: {
        "client-id": config.clientID,
...
export async function getClips(dispatch, OAuth, streamerId) {
  const twitchApi = "https://api.twitch.tv/helix/clips?broadcaster_id=" + streamerId
    + "&first=1";
  axios
    .get(twitchApi, {
      headers: {
